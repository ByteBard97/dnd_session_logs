...
lualatex flyer.tex
pdftoppm -png flyer.pdf flyer-image

# Check for the output file and report status
if [ -f "flyer.pdf" ]; then
  echo "SUCCESS: flyer.pdf has been created successfully."
else
  echo "FAILURE: Could not create flyer.pdf. Please check the logs above for errors."
  exit 1
fi
